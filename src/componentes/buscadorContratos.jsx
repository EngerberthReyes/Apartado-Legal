...
const BuscadorContratos = ({ textosFiltrados }) => {
  ...
  const { data: contratosData, error: contratosError } = useSWR(
    "http://localhost:3000/api/ctss_contratos",
    fetcher,
    {
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ fontSize: "15px" }}>Buscador de Contratos en Servicios:</p>
        <input
          ref={inputRef}
          className={`buscador ${stylesBusqueda.buscador}`}
          placeholder="Ejem: Nombre o Fecha del Contrato Sin '-'"
          type="text"
