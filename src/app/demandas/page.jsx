...
import ContenedorContratos from "@/componentes/demandas/contenedorDeContratos.jsx";
import ListadoDemandas from "@/componentes/demandas/ListadoDemandas.jsx";
...
const Index = () => {
  ...
  const [buttonText, setButtonText] = useState("Crear Demanda");
  ...
  const redireccionarNuevoContrato = () => {
    ...
    setButtonText((prevText) =>
      prevText === "Crear Demanda" ? "Ampliar Sección" : "Crear Demanda"
    );
  };
  ...
  const loadingData = async () => {
    ...
    try {
      ...
      const resultadoObjeto = responseDos.data.map((datosGet) => {
        if (datosGet.fecha_emision) {
          const fechaCompletaInicio = datosGet.fecha_emision;
          const fechaSinHoraInicio = fechaCompletaInicio.split("T")[0];
          const partesFechaInicio = fechaSinHoraInicio.split("-");
          datosGet.fecha_emision = `${partesFechaInicio[2]}-${partesFechaInicio[1]}-${partesFechaInicio[0]}`;
        }
        console.log(datosGet.fecha_emision);
        return datosGet;
      });
  ...
  return (
    <>
      <head>
        <title>Actualización de Demandas</title>
      ...
      <body className={styles.body}>
        <header className={styles.cabezera}>
          ...
          >
            ...
            />
            <h1 className={styles.tituloHeader}>Actualización de Demandas</h1>
          </section>
            ...
            <li className={styles.dropdown__list}>
              ...
              >
                ...
                <span class="dropdown__span">
                  Contratos a Servicios y Seguros
                </span>
                <Image
                  width={20}
                  height={20}
